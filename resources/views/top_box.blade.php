        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            @if(Session::has('account'))
                ...
                <a class="btn btn-outline-light me-2" href="{{ route('msg.create') }}"><ion-icon class="fs-5 align-text-bottom" name="add-outline"></ion-icon>新增留言</a>
                        <a class="btn btn-outline-light me-2" href="{{ route('member.edit', Session::get('account')) }}"><ion-icon class="fs-5 align-text-bottom" name="settings-outline"></ion-icon> 設定</a>
                <a class="btn btn-outline-light me-2" href="{{ route('member.logout') }}">登出</a>
            @else
                <ul class="navbar-nav me-auto mb-2 mb-md-0">
                    <li class="nav-item">
                        <span class="nav-link active" aria-current="page">請登入</span>
                    </li>
                </ul>
                <a class="btn btn-outline-light me-2" href="{{ route('member.index') }}">登入</a>
